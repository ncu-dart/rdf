...
# Last Modified: Sat 28 Apr 2018 06:41:15 AM CST
    ...
    def train(self, ratings, validate_ratings=None, show_process_rmse=True):
        ...
        for epoch in range(self.n_epochs):
            ...
            for u in range(self.n_users):
                user_denom[u, :] += self.n_user_rating[u] * self.lmbda_latent * self.P[u, :]
                self.P[u, :] *= user_num[u, :] / user_denom[u, :]

            for i in range(self.n_items):
                item_denom[i, :] += self.n_item_rating[i] * self.lmbda_latent * self.Q[i, :]
